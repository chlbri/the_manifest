...
var sembear = require('sembear');
var validateNpmPackageName = _interopDefault(require('validate-npm-package-name'));
var parseGithubUrl = _interopDefault(require('parse-github-url'));
...
var getPackageJson = _interopDefault(require('package-json'));
        ...
        if (deps) {
          for (var depName in deps) {
            if (!allPackages.has(depName)) {
              ...
              if (!highestExternalRange || sembear.highest([highestExternalRange, deps[depName]]) === deps[depName]) {
...
var INVALID_DEV_AND_PEER_DEPENDENCY_RELATIONSHIP = makeCheck({
  ...
  validate: function validate(workspace, allWorkspaces) {
    ...
    if (peerDeps) {
      for (var depName in peerDeps) {
        ...
        semver__default.validRange(peerDeps[depName]) && !sembear.upperBoundOfRangeAWithinBoundsOfB(devDeps[depName], peerDeps[depName])) {
  ...
  print: function print(error) {
    ...
    return "".concat(error.workspace.packageJson.name, " has a peerDependency on ").concat(error.dependencyName, " but the range specified in devDependency is not greator than or equal to the range specified in peerDependencies");
  }
});
...
var checks = [EXTERNAL_MISMATCH, INTERNAL_MISMATCH, INVALID_DEV_AND_PEER_DEPENDENCY_RELATIONSHIP, INVALID_PACKAGE_NAME, MULTIPLE_DEPENDENCY_TYPES, ROOT_HAS_DEV_DEPENDENCIES, UNSORTED_DEPENDENCIES, INCORRECT_REPOSITORY_FIELD];
    ...
    return _regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return fs.readFile(path.join(pkg.dir, "package.json"), "utf-8");

          case 2:
            pkgRaw = _context.sent;
            indent = detectIndent(pkgRaw).indent || "  ";
            return _context.abrupt("return", fs.writeFile(path.join(pkg.dir, "package.json"), JSON.stringify(pkg.packageJson, null, indent) + (pkgRaw.endsWith("\n") ? "\n" : "")));
    ...
    return _regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return getPackages.getPackages(cwd);
  ...
  try {
    for (var _iterator = checks[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      if (check.type === "all") {
        ...
        try {
          for (var _iterator2 = allWorkspaces[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            var _step2$value = _slicedToArray(_step2.value, 2),
                workspace = _step2$value[1];
      ...
      if (check.type === "root") {
        ...
        if (shouldFix && check.fix !== undefined) {
          ...
          try {
            for (var _iterator6 = _errors[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
