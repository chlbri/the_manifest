...
var EXTERNAL_MISMATCH = makeCheck({
  ...
  print: function(error) {
    return null === error.devVersion ? "".concat(error.workspace.packageJson.name, " has a peerDependency on ").concat(error.dependencyName, " but it is not also specified in devDependencies, please add it there.") : "".concat(error.workspace.packageJson.name, " has a peerDependency on ").concat(error.dependencyName, " but the range specified in devDependency is not greator than or equal to the range specified in peerDependencies");
  }
...
}), checks = [ EXTERNAL_MISMATCH, INTERNAL_MISMATCH, INVALID_DEV_AND_PEER_DEPENDENCY_RELATIONSHIP, INVALID_PACKAGE_NAME, MULTIPLE_DEPENDENCY_TYPES, ROOT_HAS_DEV_DEPENDENCIES, UNSORTED_DEPENDENCIES, INCORRECT_REPOSITORY_FIELD ], ExitError = function(_Error) {
...
function _runCmd() {
  return (_runCmd = _asyncToGenerator(_regeneratorRuntime.mark(function _callee(args, cwd) {
    var _ref, packages, root, matchingPackages, _ref2, code;
    return _regeneratorRuntime.wrap(function(_context) {
      for (;;) switch (_context.prev = _context.next) {
       case 0:
       ...
       case 2:
        if (_ref = _context.sent, packages = _ref.packages, root = _ref.root, !((matchingPackages = packages.filter(function(pkg) {
       ...
       case 16:
        return _context.next = 18, spawn("yarn", args.slice(1), {
...
var defaultOptions = {
  ...
  var hasErrored = !1, requiresInstall = !1, _iteratorNormalCompletion = !0, _didIteratorError = !1, _iteratorError = void 0;
  try {
    for (var _step, _iterator = checks[Symbol.iterator](); !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = !0) {
      var check = _step.value;
      if ("all" === check.type) {
        var _iteratorNormalCompletion2 = !0, _didIteratorError2 = !1, _iteratorError2 = void 0;
        try {
          for (var _step2, _iterator2 = allWorkspaces[Symbol.iterator](); !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = !0) {
            var workspace = _slicedToArray(_step2.value, 2)[1], errors = check.validate(workspace, allWorkspaces, rootWorkspace, options);
            if (shouldFix && void 0 !== check.fix) {
              ...
              try {
                for (var _step4, _iterator4 = errors[Symbol.iterator](); !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = !0) {
                  var _error = _step4.value;
      ...
      if ("root" === check.type) {
        ...
        if (shouldFix && void 0 !== check.fix) {
          ...
          try {
            for (var _step6, _iterator6 = _errors[Symbol.iterator](); !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = !0) {
              var _error3 = _step6.value;
            ...
            try {
              _iteratorNormalCompletion6 || null == _iterator6.return || _iterator6.return();
            } finally {
